...
- [ ] Step 5: 验证与回滚保障 ⚠️ REQUIRED
  ...
  - [ ] 5.3 人工复查 `pnpm-workspace.yaml` 中 `allowBuilds` 每个 key 的 value 是否为合法的 boolean（`true`/`false`），而非占位符字符串。发现占位符立即修正。
...
- 重点检查以下高风险面：
  ...
  - 构建依赖相关配置是否统一到 `allowBuilds` 语义。**关键规则**：
    - v11 要求 `allowBuilds` 和 `onlyBuiltDependencies` **必须配对出现**，单独存在任何一个都无效。两者都缺失会拦截所有构建脚本。
    - 正确写法示例：
      ```yaml
      allowBuilds:
        esbuild: true
      onlyBuiltDependencies:
        - esbuild
      ```
    - 配置位置在 `pnpm-workspace.yaml`（非 workspace 项目也适用）。
  ...
  - ⚠️ **校验 `allowBuilds` 值均为合法 boolean**：`allowBuilds` 下的 value **必须是 `true` 或 `false`**，不能是字符串占位符（如 `"set this to true or false"`）。占位符文本在 YAML 语法上合法（被解析为字符串），但 pnpm 不会将其解释为允许构建，`pnpm install` 仍会报错。
...
- [ ] 已人工复查 `allowBuilds` 的所有 value 是否为 `true`/`false`，无占位符字符串残留。
