...
- [ ] Step 1: 做迁移基线与目标确认 ⚠️ REQUIRED
  ...
  - [ ] 1.2 盘点仓库中的 pnpm 相关配置入口：`package.json`、`pnpm-lock.yaml`、`pnpm-workspace.yaml`、`.npmrc`、`.github/workflows/*`。
...
- [ ] Step 3: 执行自动化迁移
  ...
  - [ ] 3.2 将机械化改动集中处理：配置字段迁移、键名重命名、packageManager 对齐、lockfile 重生成。
  - [ ] 3.3 对 CI 里的 pnpm 版本策略做显式化，避免 `latest` 漂移。
...
- [ ] Step 5: 验证与回滚保障 ⚠️ REQUIRED
  - [ ] 5.1 运行最小充分质量门；涉及 CI/锁文件/配置迁移时升级为完整质量门。
  - [ ] 5.2 输出迁移报告：变更文件、人工遗留项、风险等级、回滚方式。
  - [ ] 5.3 若质量门失败，优先回退到最近稳定提交并拆分批次重试。
...
- [ ] 已完成质量门验证，并输出人工遗留项与风险说明。
- [ ] CI 中 pnpm 版本策略已显式可复现。
