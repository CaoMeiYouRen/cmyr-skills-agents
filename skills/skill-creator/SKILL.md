...
- [ ] Step 7: 选择评测模式 (conditional)
  ...
  - [ ] 7.2 复杂技能用 references/schemas.md、scripts/run_eval.py、scripts/aggregate_benchmark.py、scripts/run_loop.py 和 eval-viewer/generate_review.py 做评测与触发优化。
- [ ] Step 8: 打包与收口 ⚠️ REQUIRED
  - [ ] 8.1 使用 scripts/quick_validate.py 做快速结构检查。
  - [ ] 8.2 需要新建技能时，使用 scripts/init_skill.py 初始化骨架。
  - [ ] 8.3 需要交付时，使用 scripts/package_skill.py 打包。
  - [ ] 8.4 总结保留、迁移、删除和兼容策略。
...
- scripts/quick_validate.py：快速校验 frontmatter 与目录结构。
- scripts/package_skill.py：打包技能。
- scripts/run_eval.py、scripts/aggregate_benchmark.py、scripts/run_loop.py：评测、聚合和触发优化。
