...
### Step 4a: 搜索引擎替代抓取（conditional）

**触发条件**：某个关键页面经所有 fetch 工具尝试后均失败（403/JS 渲染/超时/404）。

**流程**：

1. **搜索引擎查找替代来源**：使用搜索工具搜索 `{页面主题} 价格 数据 2026` 或 `{页面标题关键词}`，查找第三方转载、汇总或引用了目标页面数据的文章。
2. **抓取替代来源**：对搜索到的替代页面执行 fetch（同样适用工具降级）。
3. **严格交叉比对**：
   - 第三方转载数据至少需要 **2 个独立来源** 交叉确认才可采纳
   - 优先选择 GitHub 仓库、技术社区（掘金/知乎）、行业媒体（钛媒体/36 氪）
   - 不同来源对同一数据的描述存在差异时，标注"存在矛盾"而非选择"多数一致"
   - 第三方数据永远标注为"第三方核实"而非"官方核实"
4. **记录来源链**：报告中明确标注数据来自哪个第三方来源，以及该来源引用了哪个原始页面。

**反模式**：
- 只找到一个第三方来源就确认数据
- 用 AI 训练数据中的"记忆"代替搜索结果
```bash
