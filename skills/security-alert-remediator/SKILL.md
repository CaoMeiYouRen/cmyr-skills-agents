...
- [ ] Step 4: 应用依赖升级
  ...
  - [ ] 4.4 若遇到 `ERR_PNPM_IGNORED_BUILDS` 错误（pnpm v11 中 `allowBuilds` 缺失）：
      ...
      - [ ] 4.4b **校验 `allowBuilds` 值是合法 boolean**：`allowBuilds` 的 value **必须是 `true` 或 `false`**。如果发现占位符文本（如 `"set this to true or false"` 或任何非 boolean 值），必须修正为 `true`。占位符虽在语法上合法（YAML 字符串），但 pnpm 不会将其解释为允许构建。
      - [ ] 4.4c 重新运行 `pnpm install --no-frozen-lockfile`，确认构建脚本已允许执行。
      - [ ] 4.4d 升级后务必提交更新后的 `pnpm-workspace.yaml`，否则 CI（GitHub Actions 等）会因缺少 `allowBuilds` 或值无效而构建失败。
